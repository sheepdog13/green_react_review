...
const BookBox = () => {
    ...
    const bookObj = useSelector((state)=>(state.booktest))
    //useDispatch를 이용해서 dispatch 가져오기
    const dispatch = useDispatch();
    return ( 
        <div>
            {/** 책 제목과 금액을 출력하세요 */}
            <h3>책제목:{bookObj.title}</h3>
            <h3>가격:{bookObj.pay}</h3>
            ...
            */}
            ...
            <input type="text" onChange={(e)=>{ setInputPay(e.target.value)}}/>
            <button onClick={()=>{dispatch(setPay(inputPay))}}>가격 수정</button>
